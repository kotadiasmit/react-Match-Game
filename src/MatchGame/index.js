...
class MatchGame extends Component {
  ...
  thumbnailClicked = id => {
    const {matchImageId, score} = this.state
    if (matchImageId === id) {
      this.setState({score: score + 1, matchImageId: this.randomImageId()})
    } else {
      this.setState({result: true})
    }
  }
  ...
  render() {
    const {activeTabId, matchImageId, score, result, timer} = this.state
    ...
    return (
      <div>
        ...
        <div className="game-container">
          {result || score === imagesList.length ? (
            <Result gameScore={score} playAgain={this.playAgain} />
          ) : (
            <div>
